...
async def main() -> None:
    ...
    async with session_factory() as session:
        ...
        # Check if user exists (now with valid email)
        result = await session.execute(select(UserModel).where(UserModel.email == 'admin@tradeflow.com'))
        ...
        result = await handler.handle(LoginUserCommand(email='admin@tradeflow.com', password='Admin@123456', tenant_id=tenant.id))
        ...
        print('  Email: admin@tradeflow.com')
        print('  Password: Admin@123456')
        print('  Tenant ID:', str(tenant.id))

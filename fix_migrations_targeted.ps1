# Targeted Schema Fix - Just add missing length_uom column
# Skip all the complex drops

Write-Host "Targeting specific schema fix for missing columns..." -ForegroundColor Cyan
Write-Host ""

& .venv\Scripts\Activate.ps1

# First downgrade to stable state
Write-Host "Downgrading to stable revision..." -ForegroundColor Yellow
alembic downgrade 48cc745d4daa
Write-Host ""

# Create a simple targeted migration
Write-Host "Creating targeted migration..." -ForegroundColor Yellow
...
$migrationPath = "alembic/versions/add_missing_columns_materials.py"
Set-Content -Path $migrationPath -Value $migrationContent
Write-Host "Created migration file: $migrationPath" -ForegroundColor Green
Write-Host ""

# Apply the migration
Write-Host "Applying targeted migration..." -ForegroundColor Yellow
